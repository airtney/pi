...
- Local end-to-end testing without a provider key: Ollama is installed with model `qwen2.5:1.5b`, and `~/.pi/agent/models.json` is preconfigured for an `ollama` provider. There is no systemd, so start the server yourself: `ollama serve &` (listens on 127.0.0.1:11434). Then run `./pi-test.sh --provider ollama --model qwen2.5:1.5b --api-key ollama [-p "..."]`.
